...
/**
 * Fluent assertions for AWS::SQS::QueuePolicy. This should be used if the resource map is extracted
 * from the AWS template. Otherwise, start with
 * {@link CDKStackAssert#containsQueuePolicy(String, String, String, String)}.
 * <p>
 * Example:
 * </p>
 * <pre>
 ...
 *            .hasQueuePolicy("examplelambdafunctionfailurequeue(.*)", "sqs:SendMessage", "examplelambdafunctionfailurequeue(.*)", "sns.amazonaws.com")
 *            .hasEffect("Allow")
...
public class QueuePolicyAssert extends
  ...
  public QueuePolicyAssert hasQueuePolicy(
    ...
    for (Map<String, Object> statement : statements) {
      ...
      resourceFound = isNoneBlank(resource) && resource.matches(policyStatementResource);
    }
    ...
    Assertions.assertThat(resourceFound)
        .isTrue();
